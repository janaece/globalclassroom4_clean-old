    ...
    if ($changepassword) {
        ...
        if (method_exists($authobj, 'change_password')) {
            ...
            $elements['country'] = array(
                'type'        => 'select',
                'title'       => "Country",
				'options' => array("ca" => "Canada", "us" => "United States"),
				'defaultvalue' => "us",
                'description' => "Country",
                'rules'       => array(
                    'required' => true,
...
function requiredfields_submit(Pieform $form, $values) {
	...
	if (isset($values['state'])) {
		...
		Doctrine_Query::create()
			...
			->where('platform_short_name = ?', $current_app_short_name)
			->andWhere('user_id = ?', $USER->get('id'))
			->execute();
    }
    ...
    foreach ($values as $field => $value) {
        if (in_array($field, array('submit', 'sesskey', 'password1', 'password2', 'username',
                                   ...
                                   'socialprofile_profileurl', 'socialprofile_hidden', 'state', 'country'))) {
            continue;
        }
        if ($field == 'email') {
            $email = $values['email'];
