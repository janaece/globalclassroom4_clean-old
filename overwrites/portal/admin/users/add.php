...
if ($authinstancecount) {
    ...
    //}
    // with
    $mhr_auth_instance = $CFG->current_app->getAuthInstanceForMhrInstitution();
    foreach ($authinstances as $authinstance)
    {
        if ($USER->can_edit_institution($authinstance->name) && $authinstance->authname == 'internal')
        {
            $options[$authinstance->id] = $authinstance->displayname. ': '.$authinstance->instancename;
        }
    }
...
$elements = array(
    ...
    'authinstance' => array(
        ...
        'options'      => $options,
        // OVERWRITE 2: replacement
        // 'defaultvalue' => 1,
        // replaced by:
        'defaultvalue' => $mhr_auth_instance->id,
        // END OVERWITE 2
        'rules'        => array('required' => true),
