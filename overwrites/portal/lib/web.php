    ...
    foreach ($checkarray as &$check) {
        if (($key = array_search('tinymce', $check)) !== false || ($key = array_search('tinytinymce', $check)) !== false) {
            if (!$found_tinymce) {
                ...
                $toolbar = array(
                    null,
                    '"toolbar_toggle | formatselect | bold italic | bullist numlist | link unlink | image media | undo redo"',
...
class Theme {
    ...
    public function header_logo($name = false) {
        // OVERWRITE 3: INSERTION
        // This overwrite is designed to try and use the header logo 
        // from the home MhrInstitution. If there isn't one, we default to
        // our old printBanner.php code to display the banner.
        global $CFG;
        $mhr_institution = $CFG->current_app->getMhrInstitution();
        $this->headerlogo = $mhr_institution->logo;
        if (empty($this->headerlogo)) 
        {
            $banner_file = $CFG->dataroot . 'gc_images/' . $CFG->current_app->getLogo();
            if (is_file($banner_file))
            {
                ...
                $data = (object) array(
                    ...
                    'note'        => $CFG->current_app->getLogo(),
                    'size'        => 0,
                );

                $imageinfo      = getimagesize($banner_file);
                $data->width    = $imageinfo[0];
                $data->height   = $imageinfo[1];
                $data->filetype = $imageinfo['mime'];
                $artefact = new ArtefactTypeProfileIcon(0, $data);
                if (preg_match("/\.([^\.]+)$/", $banner_file, $saved)) 
                {
                ...
                copy($banner_file, $directory . $id);

                set_field('institution', 'logo', $id, 'name', $mhr_institution->name);
                $this->headerlogo = $id;
                db_commit();
            }
        }
...
function clean_html($text, $xhtml=false) {
    ...
    $def->addElement(
      ...
      array('width' => 'Number','height' => 'Number','id' => 'ID','controls' => 'Enum#controls','preload' => 'Enum#none')
    );
    $def->addElement(
      'audio',   // name
      'Block',     // content set
      'Flow',      // allowed children
      'Common',    // attribute collection
      array('src' => 'URI','type' => 'CDATA','controls' => 'Enum#controls')
    );
    $def->addElement(
      'source',   // name
      'Inline',     // content set
